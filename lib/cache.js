const QUERY_TTL_MS = () => queryTtlMs;
let queryTtlMs = 15 * 60 * 1000;
const TIMELINE_TTL_MS = 7 * 24 * 60 * 60 * 1000;

// Cache freshness window is user-configurable (Settings → Query cache TTL, minutes).
// 0 disables query caching entirely (every pull hits the API).
function setQueryTtlMinutes(min) {
  const n = Math.round(Number(min));
  if (Number.isFinite(n) && n >= 0) queryTtlMs = n * 60 * 1000;
}
...
export { QUERY_TTL_MS, setQueryTtlMinutes, queryKey, isFreshQuery, timelineNeedsFetch, getQuery, putQuery, getTimelines, putTimelines, purgeExpired, clearAll };
