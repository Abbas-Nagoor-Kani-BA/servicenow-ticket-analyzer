...
export function pickCurNotInOptions(options, cur) {
  return !options.some(x => String(x).toLowerCase() === String(cur).toLowerCase());
}

export function pickLabelOf(v, cur, curNotInOptions) {
  return v === "" ? "— clear —"
    : v + (cur && v === cur && curNotInOptions ? " · current" : "");
}

export function pickBaseLabel(v) {
  return v === "" ? "— clear —" : v;
}

export function pickAcronymOf(s) {
 ...
 */
export function applyPickFilter(entries, q, refVal, cur) {
 ...
 */
export function paintPickItems(listEl, foot, items, activeIdx, renderItem) {
  listEl.innerHTML = "";
...
export function scrollActiveIntoView(listEl, activeIdx) {
