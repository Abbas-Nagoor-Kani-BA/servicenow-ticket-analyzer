...
(async () => {
  ...
  let c = mkClient(t);
  const progress = [];
  let got = await c.fetchTimelineEvents(["AAA", "BBB"], [], p => progress.push(p));
  ...
  check("all parsed changes kept when no filter fields requested",
    got.AAA.some(e => e.field === "state") && got.AAA.some(e => e.field === "close_notes"));
  const filtered = await c.fetchTimelineEvents(["AAA"], ["state"], () => {});
  check("explicit field list still filters", filtered.AAA.every(e => e.field === "state"));
