#!/usr/bin/env node
import { QUERY_TTL_MS, queryKey, isFreshQuery, timelineNeedsFetch } from "../lib/cache.js";
...
const TTL = QUERY_TTL_MS();
check("at TTL edge", isFreshQuery({ at: now - TTL, records: [1] }, now), false);
check("expired", isFreshQuery({ at: now - TTL - 1, records: [1] }, now), false);
check("missing records array", isFreshQuery({ at: now }, now), false);
check("default ttl 15 min", TTL, 15 * 60 * 1000);
